...
const createSchemaItem = (
  ...
  switch (schemaItem.kind) {
    ...
    case 'list':
      return S.listItem()
        .id(
          [market.name.toLowerCase(), schemaItem.title.toLowerCase().replaceAll(` `, `-`)].join(
            `-`,
          ),
        )
...
const createSchemaItemChildren = (S: StructureBuilder, schemaItem: SchemaItem, market: Market) => {
...
const createSchemaItemList = (
  ...
  const itemTitle = [market.name, schemaItem.title, languageTitle].filter(Boolean).join(` `)
...
export const structure = (S: StructureBuilder, context: ConfigContext, marketName?: string) => {
