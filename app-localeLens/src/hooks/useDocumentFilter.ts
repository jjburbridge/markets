...
export const useDocumentFilter = () => {
  const { status, defaultLanguage } = useApp();
    ...
    switch (status) {
      case 'partially-translated':
        ...
        return defaultLanguage
          ? `language == "${defaultLanguage}" && count(*[_type == "translation.metadata" && references(^._id)].translations[]) > 0 && count(*[_type == "translation.metadata" && references(^._id)].translations[]) < count(*[_type == "locale"])`
          : `count(*[_type == "translation.metadata" && references(^._id)].translations[]) > 0 && count(*[_type == "translation.metadata" && references(^._id)].translations[]) < count(*[_type == "locale"])`;

      case 'fully-translated':
        // Only show documents in the selected language that are fully translated
        return defaultLanguage
          ? `language == "${defaultLanguage}" && count(*[_type == "translation.metadata" && references(^._id)].translations[]) == count(*[_type == "locale"])`
          : `count(*[_type == "translation.metadata" && references(^._id)].translations[]) == count(*[_type == "locale"])`;
