...
import { Button, Card, Flex, Grid, Spinner, Stack, Switch, Text } from '@sanity/ui';
...
const Documents = () => {
  ...
  const handleDocumentClick = (post: { documentId: string; _id: string; _rev: string; [key: string]: any }) => {
    if (isBatchMode) {
  ...
  return (
    <Grid columns={6}>
      ...
      <Card columnStart={1} columnEnd={4} className="max-h-[92vh] overflow-y-auto flex flex-col">
        <Suspense fallback={<Loading />}>
          {/* Fixed header */}
          <div className="bg-white z-10 pt-4 px-4 pb-2 border-b border-gray-200 flex-shrink-0 sticky z-0 top-0">
              <Stack space={3}>
                <StatusSelector />
                {(status === 'untranslated' || status === 'partially-translated' || status === 'all') && (
                  <Card padding={3} radius={2} tone="transparent">
                    <Flex gap={3} align="center"  justify="space-between">
                      <Flex align="center" gap={3}>
                      <Text size={2} weight="medium">Batch Mode</Text>
                      <Switch
                        checked={isBatchMode}
          ...
          <div ref={scrollContainerRef} >
            <div className="pt-2 px-4">
              <ul>
              {data?.map((post) => {
              const isDetailSelected = selectedPost?.documentId === post.documentId;
              const isBatchSelected = isBatchMode && selectedDocuments.includes(post.documentId);
              return (
                <li key={post.documentId} className={'border-b border-gray-200 dark:border-gray-200'}>
                  <button
                    ...
                    className={`w-full text-left justify-start p-3 border-none outline-none trasnsition-background duration-200
                      ...
                      ${isDetailSelected && !isBatchMode
                         ...
                         : isBatchSelected
                           ...
                           : !shouldDisableDocumentSelection && 'hover:bg-gray-55 hover:bg-blue-300/60'
                      }
                    `}
                  >
      ...
      <Card borderLeft columnStart={4} columnEnd={7} className="overflow-hidden flex flex-col">
        <Suspense fallback={<Loading />}>
          ...
          {isBatchMode && (
            selectedDocuments.length > 0 ? (
              <Stack padding={4} space={4}>
                <Text size={2} weight="semibold">Batch Translation Details</Text>
                <BatchTranslationPanel />
              </Stack>
            ) : (
              <Stack padding={4} space={4} className="h-full justify-center items-center">
